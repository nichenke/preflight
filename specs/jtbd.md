...
### S5 — Adopter, J5
*As an adopter installing preflight in a new project (or updating across multiple projects), I want minimal first-time setup ceremony, no per-project install command afterward, automatic updates, and a clear pattern for adding project-specific rules or doc-types alongside the kernel without forking, so I can get current preflight + my additions without copying files by hand or maintaining a rebase loop.*
- Acceptance: first-time per-user setup is at most two commands (specifics deferred to the delivery-shape ADR); per-project install is zero commands once the user setup is done; updates do not overwrite project-local additions; project-specific additions live in a stable location separate from the kernel content; an adopter can answer *"is my preflight up to date?"* and *"what additions does this project layer on top?"* with one inspection each.
