...
- **SC-004**: Over a multi-phrase fixture containing two principles, each embedding two distinct implementation-detail shapes (four shapes total), the reviewer produces four CONST-R04 findings — one per offending phrase, not one per principle. This validates the rule's phrase-level flagging claim ("Each offending phrase is flagged independently"); a reviewer that truncates to principle-level granularity fails this SC even if SC-001..SC-003 pass.
