...
- **SC-004**: Over a multi-phrase fixture containing three principles embedding 2, 2, and 3 distinct implementation-detail shapes respectively (seven shapes total), the reviewer produces seven CONST-R04 findings — one per offending phrase, not one per principle, and not capped at any fixed per-principle count. This validates the rule's phrase-level flagging claim ("Each offending phrase is flagged independently. The reviewer does not stop at the first shape") at both n=2 and n=3 shape counts per principle; a reviewer that truncates to principle-level granularity or caps at two findings per principle fails this SC even if SC-001..SC-003 pass.
