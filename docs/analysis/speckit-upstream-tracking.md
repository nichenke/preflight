...
| Directory-traversal block in command write paths (#2296) | **(i) no-impact**; reclassified: 2026-04-24 (B2-follow-up against v0.8.0, which supersets v0.7.5) | Security hardening — blocks extensions from writing outside their owned paths. Verified: full `specify preset add --dev` + `specify extension add --dev` cycle succeeds against v0.8.0 without dir-traversal errors; all preflight reviewer rule + agent + command files live under `extensions/preflight/` with no `..` path components. Previous placement of #2296 under v0.7.4 was incorrect; `git tag --contains 569d18a` confirms first release is v0.7.5. |
