...
The asymmetry has persisted unchanged since its introduction on 2026-03-04 (~7 weeks) through at least six tagged releases (v0.6.0 → v0.7.4) and into v0.7.5-dev on origin/main as of 2026-04-22. The persistence interval is short in absolute terms, but the combination of (a) multiple tagged releases without a fix, (b) community reporting framed as a feature request rather than a bug, and (c) maintainer-closed #2279 ("not a bug") makes the advisory reading load-bearing — not duration alone.
...
- The template's structural framing of each block is qualitatively different:
  ...
  - After-hook: lives as the final numbered step in the Outline, **after** an explicit "Stop and report: Command ends…" step (observed in `templates/commands/plan.md` at spec-kit `c52ea23`). Structurally a post-completion notification.
...
- The asymmetry persists unchanged on origin/main as of `c52ea23` (2026-04-22), which is after release v0.7.4. Since its introduction on 2026-03-04 (~7 weeks), multiple tagged releases have shipped without anyone upstream adding the "Wait for the result" line to after-hook blocks — and the two closed community reports (#2279 "not a bug", #2104 reframed as feature request) confirm upstream's position on the behavior rather than merely indicating absent attention.
...
Prior preflight hypothesis (deleted `docs/analysis/2026-04-14-speckit-after-hook-execution-bug.md`; see git log) labeled this asymmetry a bug and sketched an upstream PR to patch all 9 templates. The mechanical observations in that doc are accurate and reused here; the label was wrong.
